...
(define-struct stare (birb lista-paip score) #:transparent) ;structura pentru stare

(define-struct birb (x y yspeed)  #:transparent) ;structura pentru pasare

(define-struct paip (x gap-y) #:transparent) ;structura pentru pipe

(define (get-initial-state)
  (stare (birb bird-x bird-initial-y 0)
         (list (paip scene-width (+ added-number (random random-threshold))))
         0))
...
(define (next-state-bird bird gravity)
  (match-let ([(birb _ y yspeed) bird])
    (struct-copy birb bird [y (+ y yspeed)] [yspeed (+ yspeed gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(birb _ _ yspeed) bird])
    (struct-copy birb bird [yspeed (* -1 momentum)])))
...
(define (change current-state pressed-key)
  (match-let ([(stare birb _ _) current-state])
    (cond [(key=? pressed-key " ")
           (struct-copy stare current-state [birb (next-state-bird-onspace birb initial-momentum)])]
          [else stare]
          )))
...
(define (move-pipes pipes scroll-speed)
  (map (lambda (p)
         (match-let ([(paip x gap-y) p])
           (struct-copy paip p [x (- x scroll-speed)] [gap-y gap-y])))
       pipes))
...
(define (add-more-pipes pipes)
  (if (< (length pipes) no-pipes)
      (let ([p (last pipes)])
        (cons
         (paip
          (+ (+ pipe-width pipe-gap) (get-pipe-x p))
     ...
     (λ (p)
       (or 
           (check-collision-rectangles
            ...
            (make-posn (get-pipe-x p) 0)
            (make-posn (+ (get-pipe-x p) pipe-width) (+ (get-pipe-gap-y p)))
            )
           (check-collision-rectangles
            (make-posn bird-x (get-bird-y bird))
            (make-posn (+ bird-x bird-width) (+ (get-bird-y bird) bird-height))
            (make-posn (get-pipe-x p) (+ (get-pipe-gap-y p) pipe-self-gap))
            (make-posn (+ (get-pipe-x p) pipe-width) ground-y))))
     pipes))
   #f
   #t))
...
(define (next-state state)
  (match-let ([(stare birb lista-paip score) state])
    (struct-copy stare state [birb (next-state-bird birb initial-gravity)]
                             [lista-paip (next-state-pipes lista-paip initial-scroll-speed)]
...
(define gap (rectangle pipe-width pipe-self-gap "solid" "white"))
...
(define (draw-frame state)
    (place-image bird-image
           ...
           (place-image ground-image
                 ...
                 (place-image (score-to-image (get-score state))
                              text-x
                              text-y
...
(define (place-pipes pipes scene)
 (foldl
   (λ (p result)
        (place-image gap
                     (+ (get-pipe-x p) (quotient pipe-width 2))
                     (+ (get-pipe-gap-y p) (quotient pipe-self-gap 2 ))
